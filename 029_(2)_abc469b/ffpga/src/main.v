...
// ===== 通信状態 =====
localparam [3:0] ST_RX_COMMAND      = 4'd0;
localparam [3:0] ST_RX_DATA         = 4'd1;
localparam [3:0] ST_RX_BURST_LENGTH = 4'd2;
localparam [3:0] ST_RX_BURST_DATA   = 4'd3;
localparam [3:0] ST_RX_ABC469B_N    = 4'd4;
localparam [3:0] ST_RX_ABC469B_S    = 4'd5;
localparam [3:0] ST_ABC469B_FLUSH   = 4'd6;
localparam [3:0] ST_WAIT_GRANT      = 4'd7;
localparam [3:0] ST_GRANT_LOW       = 4'd8;
localparam [3:0] ST_TX_RESPONSE     = 4'd9;
...
// ABC469Bは文字列全体を保存せず、3文字窓と回答だけを保持する
reg [7:0] abc469b_n;
reg [7:0] abc469b_received;
reg [7:0] abc469b_answer;
reg       abc469b_left_empty;
reg       abc469b_center_empty;
reg       abc469b_input_error;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        protocol_error     <= 1'b0;
        abc469b_n           <= 8'h00;
        abc469b_received    <= 8'h00;
        abc469b_answer      <= 8'h00;
        abc469b_left_empty  <= 1'b0;
        abc469b_center_empty <= 1'b0;
        abc469b_input_error <= 1'b0;
        burst_length       <= 9'd0;
        ...
        case (state)
            ST_RX_COMMAND: begin
                ...
                if (parallel_clk_rise) begin
                    if (!rx_high_pending) begin
                        ...
                        end else if ({rx_high_nibble, data_in} == 8'h03) begin
                            abc469b_n           <= 8'h00;
                            abc469b_received    <= 8'h00;
                            abc469b_answer      <= 8'h00;
                            abc469b_left_empty  <= 1'b0;
            ...
            ST_RX_ABC469B_N: begin
                // ABC469BのNを受信し、有効なら左端の仮想'x'を設定する
                if (parallel_clk_rise) begin
                    if (!rx_high_pending) begin
                        rx_high_nibble  <= data_in;
                        rx_high_pending <= 1'b1;
                        ...
                        rx_high_pending <= 1'b0;
                        abc469b_n       <= {rx_high_nibble, data_in};
                        tx_is_burst     <= 1'b0;
                        if ({rx_high_nibble, data_in} < 8'd1 ||
                            {rx_high_nibble, data_in} > 8'd100) begin
                            ...
                            abc469b_received    <= 8'h00;
                            abc469b_answer      <= 8'h00;
                            abc469b_left_empty  <= 1'b1;
                            abc469b_center_empty <= 1'b0;
                            abc469b_input_error <= 1'b0;
                            state               <= ST_RX_ABC469B_S;
                        end
                    end
                end
            end

            ST_RX_ABC469B_S: begin
                // 受信した新しい文字を右端として3文字窓を1文字進める
                if (parallel_clk_rise) begin
                    if (!rx_high_pending) begin
                        rx_high_nibble  <= data_in;
                        rx_high_pending <= 1'b1;
                        ...
                        rx_high_pending <= 1'b0;
                        abc469b_received <= abc469b_received + 8'd1;

                        if ({rx_high_nibble, data_in} != 8'h6F &&
                            {rx_high_nibble, data_in} != 8'h78)
                            abc469b_input_error <= 1'b1;

                        if (abc469b_received == 8'd0) begin
                            // 最初の実文字は窓の中央へ保持する
                            abc469b_center_empty <=
                                ({rx_high_nibble, data_in} == 8'h78);
                        end else begin
                            if (abc469b_left_empty &&
                                abc469b_center_empty &&
                                {rx_high_nibble, data_in} == 8'h78)
                                abc469b_answer <= abc469b_answer + 8'd1;
                            abc469b_left_empty <= abc469b_center_empty;
                            abc469b_center_empty <=
                                ({rx_high_nibble, data_in} == 8'h78);
                        end

                        if (abc469b_received + 8'd1 == abc469b_n)
                            state <= ST_ABC469B_FLUSH;
                    end
                end
            end

            ST_ABC469B_FLUSH: begin
                // 右端の仮想'x'で最後の実文字を判定し、最終値を返信する
                tx_is_burst   <= 1'b0;
                tx_has_result <= 1'b1;
                if (abc469b_input_error) begin
                    tx_status      <= STATUS_PROTOCOL_ERROR;
                    tx_result      <= 8'h00;
                    protocol_error <= 1'b1;
                end else begin
                    tx_status <= STATUS_OK;
                    tx_result <= abc469b_answer +
                        ((abc469b_left_empty && abc469b_center_empty) ?
                         8'd1 : 8'd0);
                    protocol_error <= 1'b0;
                end
