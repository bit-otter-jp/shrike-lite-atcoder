(* top *) module main (
    ...
    localparam [5:0] C_INPUT_SQUARE_START   = 6'd4;
    ...
    function [29:0] add_mod;
        ...
        begin
            ...
            if (sum >= {1'b0, MOD})
                reduced = sum - {1'b0, MOD};
            else
                reduced = sum;
            add_mod = reduced[29:0];
        end
    endfunction
        ...
        if (!rst_n) begin
            ...
            case (calc_state)
                ...
                C_INPUT_PAIR_WAIT: begin
                    if (mul_done) begin
                        pair_sum  <= add_mod(pair_sum, mul_product);
                ...
                C_INPUT_SQUARE_WAIT: begin
                    if (mul_done) begin
                        square_sum <= add_mod(square_sum, mul_product);
                        prefix_sum <= add_mod(old_prefix, x_reg);
                ...
                C_FINAL_PAIR_WAIT: begin
                    if (mul_done) begin
                        term_pair  <= add_mod(mul_product, mul_product);
