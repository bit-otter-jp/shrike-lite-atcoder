...
module abc471e_baseline_tb;
    ...
    task automatic test_three_states;
        reg ok;
        reg wait_ok;
        reg [7:0] status;
        reg [31:0] result;
        ...
        begin
            ...
            ok = ok && wait_ok && dut.prefix_sum == 30'd1 &&
                 dut.square_sum == 30'd1 && dut.pair_sum == 30'd0;

            send_word(test_a[1]);
            wait_for_input_count(2, wait_ok);
            ok = ok && wait_ok && dut.prefix_sum == 30'd3 &&
                 dut.square_sum == 30'd5 && dut.pair_sum == 30'd2;

            send_word(test_a[2]);
            wait_for_input_count(3, wait_ok);
            ok = ok && wait_ok && dut.prefix_sum == 30'd6 &&
                 dut.square_sum == 30'd14 && dut.pair_sum == 30'd11;

            poll_and_read(status, result, reply_ok);
            ok = ok && reply_ok && status == 8'h80 && result == 32'd50;
            if (!ok)
                $display("  states prefix=%0d square=%0d pair=%0d status=%02x result=%0d",
                         dut.prefix_sum, dut.square_sum, dut.pair_sum,
                         status, result);
            record_result("three_state_updates_and_hand_example", ok);
        end
    endtask
    ...
    initial begin
        ...
        if (test_pass != test_total)
            $fatal(1, "ABC471E baseline simulation failed");
        $finish;
    end
