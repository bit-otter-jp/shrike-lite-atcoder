(* top *) module main #(
    ...
    wire [WIDTH:0] comb_factor_wide =
        {1'b0, n_reg} - 1'b1 - {1'b0, scratch3} +
        {1'b0, scratch1};
    wire [WIDTH-1:0] comb_factor = comb_factor_wide[WIDTH-1:0];
                    ...
                    if (rx_data_strobe) begin
                        ...
                        if ((VALUE_BYTES == 1) || value_phase) begin
                            ...
                            if (invalid_a_event) begin
                                ...
                                x_busy         <= 1'b1;
                                scratch3       <= scratch3 + 1'b1;
                                if ((scratch3 + 1'b1) == n_reg)
                                    proto_state <= P_WAIT_RESULT;
                            end
                        end else begin
                            rx_high_byte <= rx_data;
                ...
                case (arith_state)
                    ...
                    A_COMB_CHECK: begin
                        if (scratch3 == {WIDTH{1'b0}}) begin
                            ...
                            alu_context <= U_INPUT_S1;
                            arith_state <= A_POW_CHECK;
                        end else if (scratch1 > scratch3) begin
                            scratch1    <= {{(WIDTH-1){1'b0}}, 1'b1};
                            scratch3    <= MOD - 2'd2;
                            alu_context <= U_INPUT;
                            arith_state <= A_POW_CHECK;
