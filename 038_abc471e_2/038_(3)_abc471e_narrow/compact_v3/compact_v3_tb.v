...
module compact_v3_tb #(
    ...
    task automatic test_reserved_payload_bytes;
        ...
        begin
            ...
            if (MOD32 > 255) begin
                ...
                brute_force_expected(3, 2, expected);
                begin_request(ok);
                send_value(3);
                send_value(2);
                send_a_stream(3);
                poll_and_read(status, result, reply_ok);
                all_ok = ok && reply_ok && status == 8'h80 &&
                         result == expected;
            end else begin
    ...
    initial begin
        ...
        test_a[1] = 2;
        test_a[2] = 3;
        run_case("hand_1_2_3_status_and_framing", 3, 2, 1'b1);
        ...
        test_a[2] = 3;
        test_a[3] = 4;
        run_case("k1", 4, 1, 1'b0);
        run_case("k_equals_n", 4, 4, 1'b0);

        test_a[0] = 0;
        test_a[1] = MOD32 - 1;
        test_a[2] = 1;
        run_case("ai_zero_and_mod_minus_one", 3, 2, 1'b0);

        for (random_case = 0; random_case < 24;
        ...
        run_error_k("k_zero", 3, 0);
        run_error_k("k_greater_than_n", 3, 4);
        ...
        if (test_pass != test_total)
            $fatal(1, "compact_v3 simulation failed");
        $finish;
    end
