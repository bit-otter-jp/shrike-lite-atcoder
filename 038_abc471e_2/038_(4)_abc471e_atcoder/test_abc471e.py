#!/usr/bin/env python3
"""Compile and test the ABC471E SystemVerilog solution with Icarus."""
...
SOURCE = ROOT / "abc471e.v"
EXECUTABLE = ROOT / "abc471e.out"
...
def brute_force(values: list[int], k: int) -> int:
    """Independent oracle: enumerate every K-element selection."""
    total = 0
    for selected in itertools.combinations(values, k):
        total += sum(selected) ** 2
    return total % MOD


def fast_oracle(values: list[int], k: int) -> int:
    """Large-case oracle based on element and unordered-pair counts."""
    n = len(values)
    squares = sum((value % MOD) ** 2 for value in values) % MOD
    pairs = 0
    prefix = 0
    for value in values:
        value %= MOD
        pairs = (pairs + prefix * value) % MOD
        prefix = (prefix + value) % MOD

    single_count = math.comb(n - 1, k - 1) % MOD
    pair_count = math.comb(n - 2, k - 2) % MOD if k >= 2 else 0
    return (single_count * squares + 2 * pair_count * pairs) % MOD
...
def run_verilog(n: int, k: int, values: list[int]) -> int:
    completed = subprocess.run(
        ["vvp", str(EXECUTABLE)],
        cwd=ROOT,
        input=format_input(n, k, values),
        text=True,
        capture_output=True,
        check=True,
    )
    output = completed.stdout.strip()
    if not output or not output.lstrip("-").isdigit():
        raise AssertionError(f"invalid Verilog output: {output!r}")
    return int(output)


def check_case(name: str, n: int, k: int, values: list[int], expected: int) -> None:
    actual = run_verilog(n, k, values)
    if actual != expected:
...
def run_tests(random_cases: int, seed: int) -> None:
    ...
    for case_index in range(random_cases):
        ...
        expected = brute_force(values, k)
        check_case(f"random case {case_index}", n, k, values, expected)
...
def write_max_case(path: Path, n: int = 200_000, k: int = 100_000) -> None:
    ...
    path.write_text(format_input(n, k, values), encoding="ascii")
    expected = fast_oracle(values, k)
    print(f"wrote {path} (N={n}, K={k}, expected={expected})")
...
def main() -> int:
    ...
    if args.write_max_case is not None:
        write_max_case(args.write_max_case.resolve())
